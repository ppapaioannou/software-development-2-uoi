		...
		showSelectedReceiptDetailsButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if (taxpayerReceiptsJList.getSelectedIndex()!=-1){
					Taxpayer taxpayer = database.getTaxpayerFromArrayList(taxpayerID);
					JOptionPane.showMessageDialog(null, taxpayer.getReceipt(taxpayerReceiptsJList.getSelectedIndex()).toString(), taxpayerReceiptsJList.getSelectedValue().toString(), JOptionPane.PLAIN_MESSAGE);
				}else{
			...
			public void actionPerformed(ActionEvent e) {
				if (taxpayerReceiptsJList.getSelectedIndex()!=-1){
					...
					if(dialogResult == JOptionPane.YES_OPTION){
						Taxpayer taxpayer = database.getTaxpayerFromArrayList(taxpayerID);
						taxpayer.removeReceiptFromList(taxpayerReceiptsJList.getSelectedIndex());
